...
import {extract_files_from_stream} from '../src/tools.js';
...
function extract_json_metadata(input, pkgname){
  return extract_files_from_stream(input, [`${pkgname}/extra/contents.json`]).then(function([contents]){
    if(!contents){
...
router.put('/api/packages/:package/:version/:type/:key', function(req, res, next){
...
router.patch('/api/packages/:package/:version/:type', function(req, res, next) {
  var user = res.locals.universe;
  var pkgname = req.params.package;
  var version = req.params.version;
